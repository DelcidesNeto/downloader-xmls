...
def GetDadosArqIni():
    ...
    if SelecionarCertificadoAutomaticamente == 'True':
        AutoSelectCert = True
    MyRecord = namedtuple('Record', ['MesAnoInicial', 'MesAnoFinal', 'PathDeDownload', 'AutoSelectCert'])
    return MyRecord(MesAnoInicial=MesAnoInicial, MesAnoFinal=MesAnoFinal, PathDeDownload=PathDeDownload, AutoSelectCert=AutoSelectCert)
...
def FazerPesquisa(Site, Data: str):
...
try:
    ...
    with sync_playwright() as pw:
        ...
        if DadosArqIni.AutoSelectCert:
            ...
            CNPJ = PegarCnpjManual(str(input('Nome/CPF/CNPJ do Certificado: ')))
        ...
        if CNPJ != CnpjSite:
            ...
            for c in range(0, QuantidadeDeMeses):
                ...
                if CNPJ != CnpjSite:
                    ...
                    exit()
                #Data = '02/2000'
                Erro1 = FazerPesquisa(Site=Site, Data=Data)
                if Erro1 != '':         
                    Erro2 = FazerPesquisa(Site=Site, Data=  Data)
                    if Erro2 != '':
                        Erro3 = FazerPesquisa(Site=Site, Data=Data)
                        if Erro3 != '':
